<?php

namespace App\Admin\Controllers;

use Encore\Admin\Controllers\AdminController;
use Encore\Admin\Layout\Content;
use Encore\Admin\Layout\Row;
use Encore\Admin\Layout\Column;
use App\Admin\Controllers\AggregateController;

class SummaryController extends AdminController
{
  /**
   * Title for current resource.
   *
   * @var string
   */
  protected $title = '集計画面';

  public function index(Content $content)
  {
    $aggregate = new AggregateController();
    $total_sale = $aggregate->sales_all_sum();
    $total_sale_formatted = number_format($total_sale, 0, '.', ',');
  
    return $content
        ->header('集計画面')
        ->description('売上合計')
        ->row(function(Row $row) use ($total_sale_formatted) {
            $row->column(6, function (Column $column) use ($total_sale_formatted) {
                $column->append(view('card', ['total_sale' => $total_sale_formatted]));
            });
        });
  }
}